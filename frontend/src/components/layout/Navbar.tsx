...
export default function Navbar() {
  ...
  return (
    <header className={cn(
      ...
      {/* Top banner */}
      <div className="bg-yellow-500 text-black text-center py-1.5 text-xs font-medium">
        🚚 Free Shipping on orders above ₦50,000 | Use code <strong>BILY10</strong> for 10% off
      </div>
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center gap-4 h-16">
          ...
          <div ref={searchRef} className="flex-1 max-w-xl relative">
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                placeholder="Search phones, laptops, gaming, accessories..."
                className="w-full bg-white/10 text-white placeholder-gray-400 rounded-full py-2.5 pl-4 pr-12 text-sm border border-white/20 focus:outline-none focus:border-yellow-500 focus:bg-white/15 transition-all"
              />
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isAuthenticated ? (
              <>
                <Link href="/wishlist" className="relative p-2 text-gray-300 hover:text-yellow-500 transition-colors">
                  <Heart className="w-5 h-5" />
                </Link>
                <Link href="/account/notifications" className="relative p-2 text-gray-300 hover:text-yellow-500 transition-colors">
                  <Bell className="w-5 h-5" />
                </Link>
                <div className="relative group">
                  <button className="flex items-center gap-1.5 p-2 text-gray-300 hover:text-yellow-500 transition-colors">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-xl shadow-xl border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    <div className="px-4 py-2 border-b border-gray-100">
                      <p className="font-semibold text-gray-900 text-sm">{user?.firstName} {user?.lastName}</p>
                      <p className="text-gray-500 text-xs truncate">{user?.email}</p>
                    </div>
                    {(user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                      <Link href="/admin" className="flex items-center gap-2 px-4 py-2 text-sm text-yellow-600 hover:bg-yellow-50 font-medium">
                        Admin Dashboard
                      </Link>
                    )}
              ...
              <Link
                href="/login"
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-300 hover:text-yellow-500 transition-colors"
              >
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-yellow-500 text-black text-[10px] font-bold rounded-full flex items-center justify-center min-w-[18px] h-[18px] px-0.5">
